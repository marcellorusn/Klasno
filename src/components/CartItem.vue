...
<template>
  <div class="flex gap-4 p-4 bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition">
    <div class="h-24 w-24 bg-gray-300 rounded-lg flex items-center justify-center flex-shrink-0">
      <img v-if="image" :src="image" :alt="productName" class="w-full h-full object-cover rounded" />
    ...
    <div class="flex-1">
      ...
      <div class="flex items-center gap-2 mb-3">
        ...
        <button @click="decrementQuantity" class="bg-gray-200 px-2 py-1 rounded hover:bg-gray-300 transition">
          −
        </button>
        <span class="px-3 py-1 bg-gray-100 rounded text-center min-w-12">{{ quantity }}</span>
        <button @click="incrementQuantity" class="bg-gray-200 px-2 py-1 rounded hover:bg-gray-300 transition">
          +
        </button>
      </div>
      ...
      <p class="text-sm text-gray-600 mb-2">
        Subtotal:
        <span class="font-bold text-blue-600">{{ (price * quantity).toFixed(2) }} RON</span>
      </p>
    </div>
